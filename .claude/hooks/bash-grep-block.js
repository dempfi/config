...
process.stdin.on('end', () => {
  ...
  const isOutputPipe = () => {
    if (!/\|/.test(cmd) || /xargs|exec/.test(cmd)) return false;
    const grepPos = cmd.search(/\b(grep|rg|ag|ack)\b/i);
    const pipePos = cmd.indexOf('|');
    if (pipePos === -1 || pipePos >= grepPos) return false;
    const beforePipe = cmd
      .substring(0, pipePos)
      .replace(/^\s*cd\s+(?:"[^"]*"|'[^']*'|\S+)\s*&&\s*/i, '')
      .trim();
    return /^(git|npm|npx|pnpm|node|echo|cat\s+\S+\.(?:json|md|txt|log|ya?ml))/i.test(beforePipe) ||
  ...
  if (isOutputPipe()) process.exit(0);
  if (isNamedFileSearch()) process.exit(0);
